...
class PromptBuilder(object):
    ...
    def __init__(self, prompt_path, add_rule = False, use_true = False, cot = False, explain = False, use_random = False, each_line = False, maximun_token = 4096, tokenize: Callable = lambda x: len(x), threshold = 0.7):
    ...
    def apply_rules(self, graph, rules, entity):
        results = []
        for rule in rules:
            res = utils.semantic_bfs(entity, rule, graph, self.threshhold)
            results.extend(res)
        return results
