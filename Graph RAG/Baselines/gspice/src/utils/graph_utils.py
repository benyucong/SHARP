...
class ProbabilisticUtilityPredictor:
    """Probabilistic model for predicting event utilities in graph traversal"""
    
    def __init__(self, config: SheddingConfig):
        self.config = config
        self.historical_utilities = []
        self.processing_times = []
        self.feature_weights = np.array([0.3, 0.25, 0.2, 0.15, 0.1])  # Learnable weights
        self.beta_dist = beta(config.probabilistic_alpha, config.probabilistic_beta)
        
    def extract_features(self, node: str, graph: nx.Graph, query_context: Dict = None) -> EventUtilityFeatures:
        """Extract features for utility prediction"""
        node_data = graph.nodes[node]
        node_embedding = node_data['embedding']
        
        # Temporal recency (simulated based on processing order)
        temporal_recency = 1.0  # Default high recency for new events
        
        # Semantic relevance to query context
        semantic_relevance = 0.5  # Default neutral relevance
        ...
        # Frequency score (based on embedding similarity to common patterns)
        frequency_score = np.mean(node_embedding) + 0.5  # Normalized to [0,1]
        frequency_score = max(0, min(1, frequency_score))
        
        # Downstream impact (estimated by local clustering coefficient)
        try:
    ...
    def predict_utility(self, features: EventUtilityFeatures) -> float:
        """Predict utility using probabilistic model"""
        # Convert features to numpy array
        feature_vector = np.array([
            features.temporal_recency,
        ...
        base_utility = np.dot(self.feature_weights, feature_vector)
        
        # Add probabilistic noise using beta distribution
        probabilistic_factor = self.beta_dist.rvs()
        
        # Combine deterministic and probabilistic components
        predicted_utility = 0.7 * base_utility + 0.3 * probabilistic_factor
        
        return max(0, min(1, predicted_utility))
    
    def update_model(self, predicted_utility: float, actual_outcome: float):
        """Update model based on observed outcomes"""
        self.historical_utilities.append((predicted_utility, actual_outcome))
        
        # Simple adaptation of feature weights based on prediction error
        if len(self.historical_utilities) > 10:
            recent_errors = [abs(pred - actual) for pred, actual in self.historical_utilities[-10:]]
            avg_error = np.mean(recent_errors)
            
            # Adapt weights to reduce error (simplified gradient-like update)
            if avg_error > 0.1:
                adjustment = self.config.adaptation_rate * (0.5 - avg_error)
                self.feature_weights *= (1 + adjustment)
                self.feature_weights /= np.sum(self.feature_weights)  # Normalize

class AdaptiveSheddingController:
    """Controller for adaptive shedding based on latency bounds"""
    
    def __init__(self, config: SheddingConfig):
        self.config = config
        self.utility_predictor = ProbabilisticUtilityPredictor(config)
    ...
    def should_shed_event(self, node: str, graph: nx.Graph, 
        ...
        """Decide whether to shed an event based on predicted utility"""
        self.total_events += 1
        
        # Extract features and predict utility
        features = self.utility_predictor.extract_features(node, graph, query_context)
        predicted_utility = self.utility_predictor.predict_utility(features)
        
        # Check if we should shed based on current threshold
        should_shed = predicted_utility < self.current_threshold
        ...
        if current_time:
            self._adapt_threshold(current_time)
            
        return should_shed, predicted_utility
    
    def _adapt_threshold(self, current_time: float):
        """Adapt shedding threshold based on latency performance"""
...
def semantic_bfs_with_shedding(start_node, target_rule, graph: nx.Graph, 
    ...
    '''
    Semantic Regular Path Query with Probabilistic Shedding
    '''
    ...
    # Initialize shedding controller
    shedding_controller = AdaptiveSheddingController(shedding_config)
    
    # Use priority queue for better path selection
    # Priority based on negative utility (higher utility = higher priority)
    queue = []
    ...
    heapq.heappush(queue, (0, 0, start_node, initial_path))  # (priority, counter, node, path)
    ...
    while queue and processed_nodes < shedding_config.max_queue_size:
        ...
        for neighbor in neighbors:
            ...
            should_shed, predicted_utility = shedding_controller.should_shed_event(
                neighbor, graph, query_context, current_time
            )
            
            if should_shed:
                continue  # Drop this event/path
            ...
            if current_path.if_accept_rel_embedding(rel_embedding):
                ...
                # Calculate priority (negative utility for min-heap)
    ...
    stats = shedding_controller.get_statistics()
    print(f"Shedding Statistics: {stats}")
