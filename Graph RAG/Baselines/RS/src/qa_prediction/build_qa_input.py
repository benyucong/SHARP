...
graph = utils.build_graph('./datasets/kb.txt')
...
class PromptBuilder(object):
    ...
    def __init__(self, prompt_path, add_rule = False, use_true = False, cot = False, explain = False, use_random = False, each_line = False, maximun_token = 4096, tokenize: Callable = lambda x: len(x), max_queue_len = 7):
    ...
    def apply_rules(self, graph, rules, entity):
        results = []
        for rule in rules:
            res = utils.semantic_bfs(entity, rule, graph, self.max_queue_len)
            results.extend(res)
        return results
    
    def direct_answer(self, question_dict):
        graph = utils.build_graph(question_dict['graph'])
        entity = question_dict['q_entity']
        rules = question_dict['predicted_paths']
        prediction = []
        if len(rules) > 0:
            retrieve_start_time = time.time()
            reasoning_paths = self.apply_rules(graph, rules, entity)
            retrieve_time = time.time() - retrieve_start_time
            for p in reasoning_paths:
                if len(p) > 0:
                    prediction.append(p[-1][-1])
        return prediction, retrieve_time
    ...
    def process_input(self, question_dict):
        ...
        if self.add_rule:
            ...
            if self.use_true:
                ...
                _, rules = utils.get_random_paths(entity, graph)
            else:
                rules = question_dict['predicted_paths']
            if len(rules) > 0:
                ...
                tracemalloc.stop()
                lists_of_paths = [p.__repr__() for p in reasoning_paths]
        ...
        return input, retrieve_time, peak_memory
