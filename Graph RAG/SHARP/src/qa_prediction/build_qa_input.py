...
class PromptBuilder(object):
    ...
    def apply_rules(self, graph, rules, entity):
        results = []
        result_paths, retrieve_time, peak_memory = utils.semantic_bfs(entity, rules, graph)
        results.extend(result_paths)
        return results, retrieve_time, peak_memory
    ...
    def process_input(self, question_dict):
        ...
        if self.add_rule:
            ...
            if self.use_true:
                ...
                rules = question_dict['predicted_paths']
            if len(rules) > 0:
                reasoning_paths, retrieve_time, peak_memory = self.apply_rules(graph, rules, entity)
