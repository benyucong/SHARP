...
class Path:
    def __init__(self, start_node, dfas: list[DFA]=None, tuples=[]) -> None:
    ...
    def add_new_tuple(self, new_tuple):
        ...
        #todo: update scores and dfas, do it here
        for id, dfa in enumerate(new_path.dfa_list):
            # if this path is already invalid, dont update the dfa state
            if new_path.scores[id] < self.threshold:
                continue
            for dfa_relation in dfa.transitions[dfa.current_state]:
                ...
                if similarity_score > relation_similarity_threshold:
                    dfa.process_one_token(dfa_relation)
                    # update scores
                    new_path.scores[id] = new_path.scores[id]* 0.4 + similarity_score * 0.6
                    new_path.visited_nodes.add(next_node)
        return new_path
    
    def if_accept_rel_embedding(self, rel_embedding):
        for id, dfa in enumerate(self.dfa_list):
            if self.scores[id] < self.threshold:
                continue
            for relation in dfa.transitions[dfa.current_state]:
    ...
    def is_accepted(self) -> bool:
        if self.if_valid():
            for dfa in self.dfa_list:
                if dfa.current_state in dfa.accept_states:
                    return True
        return False
...
def semantic_bfs(start_node, target_rules, graph: nx.Graph, max_hop=3):
    '''
    Semantic Regular Path Query
    '''
    result_paths = []
    dfa_list = construct_combined_DFA(target_rules)
    queue = deque([(start_node, Path(start_node, dfa_list))])
    ...
    tracemalloc.stop()
    return result_paths, retrieve_time, peak_memory
