...
auto Predicate::Verify(EventVector &seq) -> bool {
  ...
  if (pr_str_ == "q0_p1") {
    // if (std::fabs(events_vec.begin()->GetAttributeValue("id") - events_vec.rbegin()->GetAttributeValue("id")) >
    // 1e-5L) {
    //   // std::cout << "id failed " << std::endl;
    //   return false;
    // }

    if (events_vec.size() == 2) {
      // std::cout << "p1 failed " << std::endl;
      return events_vec[0].GetAttributeValue("v") < events_vec[1].GetAttributeValue("v");
    }

    if (events_vec.size() == 4) {
      // std::cout << "p2 failed " << std::endl;
      return events_vec[1].GetAttributeValue("v") + events_vec[2].GetAttributeValue("v") <
    ...
    if (events_vec.size() == 6) {
      // Assuming `events_vec` is a vector of objects that have a `GetAttributeValue()` method
      auto lat1 = events_vec[3].GetAttributeValue("x") * M_PI / 180.0L;
      auto lon1 = events_vec[3].GetAttributeValue("y") * M_PI / 180.0L;
      auto lat2 = events_vec[4].GetAttributeValue("x") * M_PI / 180.0L;
      auto lon2 = events_vec[4].GetAttributeValue("y") * M_PI / 180.0L;

      constexpr long double earth_radius_m = 6371.0L * 1000.0L;  // Earth's radius in meters

      // Compute distance using Spherical Law of Cosines
      auto distance = earth_radius_m * std::acos(std::sin(lat1) * std::sin(lat2) +
