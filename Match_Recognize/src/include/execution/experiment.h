...
class EventBuffer {
 public:
  ...
  int pattern_length_ = 100;
  float darling_threshold_ = 1e-5;
...
class Experiment {
 public:
  ...
  Experiment(int plan_choice, int query_choice, time_t q0_bound, time_t q1_bound, int shed_method, uint64_t q0_window,
             uint64_t q1_window, int shedding_ratio, int pattern_length);
  ...
  // auto ReadEventsfromFileQ1(const std::string &file_path) -> void;
  auto ReadEventsfromFileQ2(const std::string &file_path) -> void;
