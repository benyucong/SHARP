...
auto LoadSheddingManager::LoadSheddingICDE(long double extend1, long double extend2,
  ...
  auto shed_quota_q1 = static_cast<uint64_t>(
      (all_buckets[1].pm_quality_bimap_.size() + all_buckets[2].pm_quality_bimap_.size()) * extend2);

  shed_quota_q0 = shed_quota_q0 * ratio * 1.5 / 100;
  shed_quota_q1 = shed_quota_q1 * ratio * 1.5 / 100;
